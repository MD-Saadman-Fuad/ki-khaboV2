...
<!-- Category Hero Banner Starts Here -->
<section class="relative bg-stone-900 text-white overflow-hidden py-16 sm:py-24 bg-cover bg-center"
         style="background-image: linear-gradient(180deg, rgba(0,0,0,0.55) 0%, rgba(0,0,0,0.65) 50%, rgba(0,0,0,0.85) 100%), url('https://images.unsplash.com/photo-1544025162-d76694265947?auto=format&fit=crop&w=1600&q=80'), url('images/bg.jpg');">
    <div class="container mx-auto px-4 text-center max-w-4xl relative z-10">
        ...
        <h1 class="text-4xl sm:text-6xl font-extrabold tracking-tight mb-4 text-white drop-shadow-md">
            Dishes in <span class="text-orange-400">"<?php echo htmlspecialchars($category_title); ?>"</span>
        </h1>
        <p class="text-stone-200 text-base sm:text-xl font-medium max-w-2xl mx-auto drop-shadow-sm leading-relaxed">
            Explore all available delicious recipes under the <?php echo htmlspecialchars($category_title); ?> category.
        </p>
    </div>
</section>
<!-- Category Hero Banner Ends Here -->
    ...
    <div class="container mx-auto px-4 max-w-6xl">
        ...
        <div class="flex flex-wrap justify-center items-center gap-2.5 mb-12">
          <a href="<?php echo SITEURL; ?>foods.php" 
             class="px-4 py-2 rounded-full font-semibold text-xs sm:text-sm bg-white hover:bg-orange-50 text-stone-700 border border-stone-200/80 transition transform hover:scale-105">
            All Dishes
          </a>
          <?php if ($cat_res && mysqli_num_rows($cat_res) > 0): ?>
            <?php while ($cat_row = mysqli_fetch_assoc($cat_res)): 
              $is_active = ($cat_row['id'] == $category_id);
            ?>
              <a href="<?php echo SITEURL; ?>category-foods.php?category_id=<?php echo $cat_row['id']; ?>" 
                 class="px-5 py-2 rounded-full font-bold text-xs sm:text-sm transition transform hover:scale-105 <?php echo $is_active ? 'bg-stone-900 text-white shadow-md' : 'bg-white hover:bg-orange-50 text-stone-700 border border-stone-200/80 shadow-2xs'; ?>">
